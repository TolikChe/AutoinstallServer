 ...
 * Класс, описывающий работу с подсистемой CMS_SRV_DB.
 */
public class SetupServiceCmsSrvDb extends SetupServiceImpl {

    // Объявляем переменную логгера
    private Logger log = LoggerFactory.getLogger(SetupServiceCmsSrvDb.class);
    ...
    public void setupSubsystem(SubsystemConfig subsystem, String destination) throws Exception {
        ...
        command = "cmd /c \"cd /d " + destination + "/Srv_Part/" + subsystem.getDistribResultDirectory() + " && install.bat \"i686-ms-win(2008)-64\"\"";
        //
        try {
            log.info(command);
